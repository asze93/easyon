...
function initSupabase() {
    ...
    supabaseClient.auth.onAuthStateChange(async (event, session) => {
        ...
        if (currentUser) {
            console.log("[Auth]: Bruger logget ind:", currentUser.id);
            await loadDashboard();
        } else {
...
async function loadDashboard(providedProfile = null) {
    ...
    // UUID-parring er 100% mere stabilt end email-parring (v40 fix)
    if (!profile && currentUser?.id) {
        for (let i = 0; i < 3; i++) {
            try {
                ...
                if (data) { 
                    profile = data; 
                    console.log("[Dashboard]: Profil indlæst via UUID:", profile.id);
                    break; 
                }
    ...
    if (profile) {
        ...
        const displayName = profile.fornavn || profile.navn || profile.email || "Bruger";
        document.querySelectorAll('.adminName').forEach(el => el.innerText = displayName);
        // Also populate identity line if it exists
        const idDisplay = document.getElementById('user-display-identity');
        if (idDisplay) idDisplay.innerText = `${profile.email || ''} • ${lowRole.toUpperCase()} • ${f?.navn || 'EasyON'}`;
    } else { 
...
// ---------------- SSCC & LOGISTICS (DIAMOND UPGRADE) 🚢 ----------------
let currentSsccManifest = [];

async function fetchSsccShipments() {
...
function previewSsccShipment(ship) {
...
// SSCC Modal logic
async function prepareSsccModal() {
    currentSsccManifest = [];
    renderSsccManifest();
    ...
    try {
        const { data } = await supabaseClient.from('lager').select('id, navn').eq('firma_id', currentFirmaId).order('navn');
...
function renderSsccManifest() {
    ...
    currentSsccManifest.forEach((item, idx) => {
        ...
        div.innerHTML = `
            ...
            <button onclick="currentSsccManifest.splice(${idx}, 1); renderSsccManifest();" style="background:transparent; border:none; color:var(--danger); cursor:pointer;">✕</button>
        `;
        container.appendChild(div);
    });
}

async function saveSsccShipment() {
    ...
    const btn = document.getElementById('btnSaveSscc');
    setLoading(btn, true);
    
    try {
        // 1. Opret hovedforsendelsen
        const { data: ship, error: sErr } = await supabaseClient.from('forsendelser').insert({
            firma_id: currentFirmaId,
            sscc_kode: ssccCode,
            leverandoer: vendor,
            status: 'afventer'
        ...
        const { error: iErr } = await supabaseClient.from('forsendelse_indhold').insert(itemsToInsert);
        if (iErr) throw iErr;
...
// ---------------- INDKØB & LEVERANDØRER (PROCUREMENT) 🛒 ----------------
...
async function fetchSuppliers() {
    ...
    if (!list) return;
    try {
        const { data, error } = await supabaseClient.from('leverandoerer').select('*').eq('firma_id', currentFirmaId).order('navn');
        if (error) throw error;
        if (!data || data.length === 0) {
            ...
            div.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div style="text-align:right;">
                        ...
                        <div style="font-size:11px; color:var(--text-muted); margin-top:4px;">${itemCount} varer • ${po.total_pris} ${po.valuta}</div>
                        ${po.status === 'sent' && !po.sscc_id ? `<button class="btn-primary" style="font-size:10px; padding:4px 8px; margin-top:5px;" onclick="convertPoToSscc('${po.id}')">Opret SSCC</button>` : ''}
                    </div>
                </div>
            `;
            list.appendChild(div);
...
async function handleSupplierSubmit(e) {
    ...
    try {
        const { error } = await supabaseClient.from('leverandoerer').insert(sup);
        if (error) throw error;
...
async function preparePoModal() {
    ...
    // Hent Leverandører
    const { data: vendors } = await supabaseClient.from('leverandoerer').select('id, navn').eq('firma_id', currentFirmaId).order('navn');
    vSelect.innerHTML = '<option value="">Vælg leverandør...</option>';
    vendors?.forEach(v => vSelect.innerHTML += `<option value="${v.id}">${v.navn}</option>`);
    
    // Hent Lager
    const { data: items } = await supabaseClient.from('lager').select('id, navn').eq('firma_id', currentFirmaId).order('navn');
...
function addPoItemToList() {
    ...
    const qty = parseInt(document.getElementById('po_add_qty').value) || 1;
    ...
    currentPoManifest.forEach((item, idx) => {
        container.innerHTML += `
            <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.03); padding:10px 16px; border-radius:10px;">
                ...
                <button onclick="currentPoManifest.splice(${idx}, 1); renderPoManifest();" style="background:transparent; border:none; color:var(--danger); cursor:pointer;">✕</button>
            </div>
        `;
    });
}
...
async function savePurchaseOrder() {
    ...
    setLoading(btn, true);
    try {
        const { data: po, error: pErr } = await supabaseClient.from('indkoeb_ordrer').insert({
            firma_id: currentFirmaId,
            leverandoer_id: vendorId,
            valuta: currency,
            status: 'sent' // Vi sætter den som sendt med det samme for demo
        }).select().maybeSingle();
        ...
        const { error: iErr } = await supabaseClient.from('indkoeb_indhold').insert(contents);
        if (iErr) throw iErr;
...
async function convertPoToSscc(poId) {
    if (!confirm("Vil du omdanne denne bestilling til en SSCC-forsendelse nu?")) return;
    
    try {
        const { data: po } = await supabaseClient.from('indkoeb_ordrer').select('*, leverandoerer(navn), indkoeb_indhold(*)').eq('id', poId).single();
        
        // 1. Opret SSCC Forsendelse
        const ssccCode = "00" + Math.floor(Math.random() * 9000000000000000 + 1000000000000000).toString(); // Simuleret SSCC
        const { data: ship, error: sErr } = await supabaseClient.from('forsendelser').insert({
            firma_id: currentFirmaId,
            sscc_kode: ssccCode,
        ...
        // 2. Kopier indhold
        const shipItems = po.indkoeb_indhold.map(i => ({
            forsendelse_id: ship.id,
            lager_id: i.lager_id,
            antal: i.antal
        }));
        await supabaseClient.from('forsendelse_indhold').insert(shipItems);
        
        // 3. Opdater PO med SSCC ID
        await supabaseClient.from('indkoeb_ordrer').update({ sscc_id: ship.id }).eq('id', poId);
